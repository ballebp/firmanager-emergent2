              ...
              {upcomingOrders.map((order, idx) => (
                <div key={idx} className="flex items-center gap-3 p-2 bg-gray-800 rounded">
                  <div className="w-12 text-center">
                    <p className="text-xs text-gray-400">{new Date(order.date).toLocaleDateString('no-NO', { day: '2-digit', month: 'short' })}</p>
                  </div>
            ...
            <div className="space-y-3">
              {recentOrders.map((order, idx) => (
                <div key={idx} className="flex items-center gap-3 p-2 bg-gray-800 rounded">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium truncate">{getCustomerName(order.customer_id)}</p>
                    <p className="text-xs text-gray-400">{new Date(order.date).toLocaleDateString('no-NO')} - {getEmployeeName(order.employee_id)}</p>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded ${order.status === 'fullført' ? 'bg-green-900/50 text-green-400' : 'bg-yellow-900/50 text-yellow-400'}`}>
                    {order.status}
                  </span>
                </div>
              ))}
